...
extern "C" float* py_readAccel(){
  float* imu_data = new float[6];
  int status = dof.begin();
  dof.readAccel();
  imu_data[0] = dof.calcAccel(dof.ax);
  imu_data[1] = dof.calcAccel(dof.ay);
  imu_data[2] = dof.calcAccel(dof.az);
  dof.readGyro();
  imu_data[3] = dof.calcGyro(dof.gx);
  imu_data[4] = dof.calcGyro(dof.gy);
  imu_data[5] = dof.calcGyro(dof.gz);
  return imu_data;
}
...
int main() {
  ...
  cout << "Status: " << status << endl;
	while (1) {
		...
		cout << ", ";
		cout << dof.calcAccel(dof.az) << " " << flush;
		...
		cout << ", ";
		cout << dof.calcGyro(dof.gz) << " " << flush;
		...
		dof.readTemp();
		cout << "T: " << (21.0 + (float)dof.temperature/8.) << " " << flush;
		//sleep(1);
	}
}
